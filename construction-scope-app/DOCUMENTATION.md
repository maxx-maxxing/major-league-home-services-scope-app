...
- Milestone 7.6.4.2 save-confirmed Documents asset retirement — 2026-07-20:
  ...
  - Documents mutations still update the visible in-memory slot/row immediately. Only operations that actually supersede an existing file use the existing immediate manual-flush path; attachment-free imports, added rows, names, and other metadata-only edits keep the existing debounce behavior.
  - Before retirement, the action rechecks the current scope for the old attachment ID or standardized path. The new `DocumentAssetRetirement` helper then requires an exact direct child of the current container's `Application Support/ScopeAssets/<scope-id>/Documents` directory with the expected attachment-ID filename prefix.
  - Directory traversal is anchored with `openat` plus `O_NOFOLLOW`; the final entry is inspected with `fstatat` plus `AT_SYMLINK_NOFOLLOW`; only a regular file can be removed with `unlinkat`. Wrong container/scope/ID, nonstandard/traversal paths, prefix collisions, directories, special files, and final or ancestor symlinks fail closed. Results are fixed and path-free, and uncertain cases leave an orphan rather than risk unrelated data.
  - `Models/SchemaModels.swift` and `schema.json` remain byte-for-byte unchanged. No Documents labels, menus, import sources, row/name rules, attachment metadata shape, PDF/pricing/JobTread behavior, general asset system, cloud/backend, or live system was redesigned.
  - Verification passed: the dedicated strict-concurrency retirement/save-retry gate, unsigned generic-iOS Debug and Release builds, Release analysis, repository-baseline persistence compatibility, persistence save health, offline JobTread read contract, JobTread Release security/artifact containment, PDF diagnostic privacy, project/plist parsing, shell syntax/executable modes, frozen hashes, and whitespace checks. A transient host source-read timeout during a concurrent security scan disappeared when the same gate was rerun alone; the isolated gate passed.
  - Independent behavior/privacy review found no regression or privacy blocker. Destructive file operations now save immediately by design, so target-device testing must still confirm that replacement/removal completion has no perceptible hitch.
  - Manual acceptance remains: exercise replace, clear, and additional-row removal through Files, Photos, and Camera on target iPhone/iPad; inject/retry a save failure; force-close/relaunch; and confirm the current/new attachment reopens while the retired file does not. Async import ordering/cancellation, durable orphan cleanup, relative-path migration, backup/import, reinstall/new-container recovery, deployment/TestFlight, and all external writes remain separate work.
...
- Milestone 2.4 Section review / completion workflow:
  - Task scope:
    ...
    - keep completion independent of required fields, validation, PDF export, pricing, and JobTread behavior
    - automatically reset a completed section to `Needs Review` when its section content changes
  - Implemented architecture:
    - added a small file-backed `SectionReviewStore` in [Persistence.swift](/Users/maxx/Documents/major-league-home-services-scope-app/construction-scope-app/Persistence/Persistence.swift) that stores completed section keys by scope UUID under Application Support workflow state
    - avoided adding a new SwiftData `JobScope` stored property, preserving the current SwiftData schema shape for this workflow-only state
    - added stable `ScopeSection.reviewStateKey` values in [RootNavigationView.swift](/Users/maxx/Documents/major-league-home-services-scope-app/construction-scope-app/Views/RootNavigationView.swift) so persisted review state is not coupled to visible section labels
  - UI behavior:
    - each section header now shows `Completed` or `Needs Review` and a lightweight `Mark Complete` / `Reconfirm` control
    - iPad sidebar section rows and compact phone section rows now show a trailing completion indicator
    - completed state is purely reassuring workflow state; sections remain freely editable and export is not gated
  - Invalidation behavior:
    - the section editor shell computes lightweight fingerprints for section-owned content
    - when a fingerprint changes for a completed section, that section's completion mark is removed and the UI returns to `Needs Review`
    - Screen Enclosure and Sunroom fingerprints are split even though they share the `Enclosure` model, so ordinary Sunroom edits do not automatically invalidate Screen Enclosure review state
  - Deferred:
    - no required-field enforcement
    - no export or validation dependency on review state
    - no section-specific checklist prompts or reviewer identity/timestamp UI beyond the small completed date
    - no JobTread, pricing, PDF, or broader persistence refactor
  - Validation:
    - `git diff --check` passed
    - `xcodebuild -project ConstructionScopeApp.xcodeproj -scheme ConstructionScopeApp -destination 'generic/platform=iOS' -derivedDataPath /tmp/ConstructionScopeAppSectionReviewDerivedData CODE_SIGNING_ALLOWED=NO build` succeeded

- Milestone 4.8.3 PDF heading hierarchy polish:
  - Task scope:
    - remove redundant page-level continuation headings generated by the compact `Scope Details` flow
    - preserve useful section-level continuation labels for sections that actually span pages
    - make section titles slightly more distinct without making them compete with the customer/document title or page heading
  - Implemented change:
    - changed continuation page creation in [PDFPreviewStubView.swift](/Users/maxx/Documents/major-league-home-services-scope-app/construction-scope-app/PDFEngine/PDFPreviewStubView.swift) so continuation pages keep the base page title instead of rendering `Scope Details (Cont. x)`
    - left section continuation labels such as `Sunroom (Cont.)` intact because those are tied to actual section overflow
    - increased section title styling from 12pt bold to 13pt semibold and gave the title line a small amount of extra measured breathing room
  - Preserved:
    - relevance filtering, compact core pagination, Existing Conditions and Scope Photos thumbnails, site diagram appendix handling, and flattened PDF rendering
    - understated Apple-like hierarchy: section titles remain below the 16pt page heading and 18pt document title
  - Validation:
    - `git diff --check` passed
    - escalated `xcodebuild -project ConstructionScopeApp.xcodeproj -scheme ConstructionScopeApp -destination 'generic/platform=iOS' -derivedDataPath /tmp/ConstructionScopeAppPDFHeadingPolishDerivedData CODE_SIGNING_ALLOWED=NO build` succeeded

- Milestone 4.8.2 Compact PDF section flow:
  - Root cause:
    - the export composition still created five fixed core `PDFPageContent` buckets after relevance filtering
    - `paginatePages` paginated each bucket independently, so a short `Job Header + Project Info` bucket could leave unused page space while the next relevant section started on a fresh page
    - section containers and row measurement were already height-driven; the excessive whitespace came primarily from the fixed core page boundaries
  - Implemented change:
    - added a compact `coreFlowPage` composition helper in [PDFPreviewStubView.swift](/Users/maxx/Documents/major-league-home-services-scope-app/construction-scope-app/PDFEngine/PDFPreviewStubView.swift)
    - reused the existing filtered core section builders, then flattened their sections into one `Scope Details` core flow before pagination
    - kept appendices such as the site diagram as separate pages when they need dedicated drawing space
  - Preserved:
    - Milestone 4.8 relevance filtering for inactive, empty, default, or hidden dependent content
    - Milestone 4.8.1 compact Existing Conditions and Scope Photos thumbnail behavior
    - existing row, thumbnail, image, header, footer, and flattened PDF drawing logic
  - Validation:
    - `git diff --check` passed
    - initial sandboxed build attempt reached Swift compilation but failed during asset catalog compilation because sandboxed CoreSimulator services were unavailable
    - escalated `xcodebuild -project ConstructionScopeApp.xcodeproj -scheme ConstructionScopeApp -destination 'generic/platform=iOS' -derivedDataPath /tmp/ConstructionScopeAppPDFCompactFlowDerivedData CODE_SIGNING_ALLOWED=NO build` succeeded

- Milestone 4.8.1 Compact PDF photo thumbnails:
  - Task scope:
    - replace text-only Existing Conditions checklist photo summary rows such as `Front of House (1)` with compact visual thumbnail groups
    - keep PDF output concise by showing only a small preview set for each relevant photo category
    - preserve Milestone 4.8 relevance filtering so empty/default/inactive sections still do not render
  - Implemented architecture:
    - extended the private PDF composition model in [PDFPreviewStubView.swift](/Users/maxx/Documents/major-league-home-services-scope-app/construction-scope-app/PDFEngine/PDFPreviewStubView.swift) with a small `PDFThumbnailGrid` payload on `PDFSection`
    - kept category relevance and image loading in export composition helpers before pagination/rendering
    - kept the low-level renderer presentational: it only measures and draws already-selected thumbnail grids
  - Rendering behavior:
    - Existing Conditions checklist categories now render as their own compact sections using the category label as the section title
    - each category shows up to three rounded thumbnails directly under the label
    - categories with more than three attached images show a small `+N more` indicator
    - general scope photos now render as a compact `Scope Photos` thumbnail section instead of one full appendix page per photo
    - site diagram remains an appendix because it needs readable drawing space
  - Deferred:
    - no broader Documents / Attachments thumbnail redesign for image files imported into document slots
    - no pricing/proposal redesign and no JobTread, persistence, or pricing-domain changes
    - no per-image captions in the compact grid unless a later production-order design pass explicitly needs them
  - Validation:
    - `git diff --check` passed
    - attempted `xcodebuild -project ConstructionScopeApp.xcodeproj -scheme ConstructionScopeApp -destination 'generic/platform=iOS' -derivedDataPath /tmp/ConstructionScopeAppPDFThumbnailsDerivedData CODE_SIGNING_ALLOWED=NO build`
    - build reached the Swift driver with no source diagnostics surfaced for the PDF thumbnail changes
