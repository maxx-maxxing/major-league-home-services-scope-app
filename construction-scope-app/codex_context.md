...
Milestones 7.6.4.1 save-failure containment, 4.8.4 PDF diagnostic privacy, 7.6.2.3 offline JobTread customer read-contract verification, 7.6.4.2 save-confirmed Documents asset retirement, and 7.6.4.3 Documents async-import identity safety are implemented and repository-verified. Preserve these checkpoints. The next priority is manual target-device acceptance for Documents/PDF/save continuity. Durable backup/import, path migration, orphan sweeping, or camera performance requires a separate plan.

Git checkpoint and authority:
- Active branch: `ai-scope-assistant`
- Verified checkpoints `4b6bce0`, `2f544e7`, `366b090`, `95a9978`, and `92e0376` (save-confirmed Documents retirement) are pushed to `origin/ai-scope-assistant`. Milestone 7.6.4.3 is the current verified bounded checkpoint; confirm `HEAD` and upstream before another slice.
...
- Milestone 7.6.4 remains in progress: 7.6.4.1–7.6.4.3 are implemented, while the device continuity matrix and lossless backup/import remain separate open work.
...
- Documents use an app-controlled encoded payload. Superseded files retire only after confirmed save through a scoped safe path; async imports adopt only through exact immutable request identity and safely retire never-adopted managed files.
...
- Documents imports are request-identified but have no durable task/progress state. Photo cancellation may leave a harmless request until later import/navigation invalidation; camera encoding/file writing remains synchronous on the main actor.
...
Milestone 7.6.4.3 verified boundary:
- `DocumentImportCoordination` binds each import to immutable request, scope, and target identity; only the exact current request for the same scope and a still-existing target may adopt.
- Presenter callbacks are request-bound and Photos captures identity before suspension. Stale success/failure cannot overwrite or dismiss a newer request; camera creation/dismissal validates first.
- Same-target clear, row removal, and editor disappearance invalidate identity. Clearing another target leaves the request active.
- Current success preserves empty-slot debounce and replacement confirmed-save behavior. Never-adopted success rechecks scope and ID/path references before scoped retirement.
- `./scripts/verify_document_import_coordination.sh` must pass with retirement/save-health, persistence compatibility, JobTread contract/security, and PDF privacy. Model/schema remain frozen.
- Manual acceptance covers rapid same/cross-target Files/Photos imports, clear/remove/navigation while loading, Camera, cancellation, error handling, file reopening, and responsiveness.
- Exclude durable task/retirement state, orphan sweeping, backup/import, path migration, reinstall recovery, camera performance, other asset systems, deployment, and external writes.
